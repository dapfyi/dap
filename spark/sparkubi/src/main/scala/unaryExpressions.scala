...
case class Resolve(child: Expression) 
    ...
    override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
        defineCodeGen(ctx, ev, c => s"d3centr.sparkubi.Resolve.method($S2FName($c))")
    }
...
case class Abs(child: Expression) extends 
    ...
    override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
        defineCodeGen(ctx, ev, c => s"d3centr.sparkubi.Abs.method($S2FName($c))")
    }
...
case class Sign(child: Expression) extends UnaryExpression {
    ...
    override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
        defineCodeGen(ctx, ev, c => s"d3centr.sparkubi.Unary.sign($S2FName($c))")
    }
